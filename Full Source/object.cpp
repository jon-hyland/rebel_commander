...
extern int PlayerKills;


CObject::CObject(ObjectType object, SpriteType object_sprite, SpriteType dying_sprite, UpType up_type, int x, int y, ObjEventItem **event_list)
{
	...
	{
		case ORANGE_BLIP_OBJECT: case GREEN_BLIP_OBJECT:
			m_nColliWidth=30;
			m_nColliHeight=30;
			m_bPivotAnimation=FALSE;
			m_nFrameInterval=100;
			m_nDyingFrameInterval=75;
			break;
		case BLUE_SLITH_OBJECT:
			m_nColliWidth=30;
			m_nColliHeight=30;
			m_bPivotAnimation=FALSE;
			m_nFrameInterval=75;
	...
	m_nLastX=m_nX;
	m_nLastY=m_nY;
	if(m_pEventList[m_nEventIndex]->iTargetX<0)
		AlterVector(m_nVectorX, m_nVectorY, m_nX, m_nY, Ship.GetX(), Ship.GetY(), m_pEventList[m_nEventIndex]->iVectorSpeed);
	else
...
void CObject::Draw(LPDIRECTDRAWSURFACE surface, DWORD time)
{
	//to do: use above time var! get rid of Timer.Elapsed()
		...
		if(GameState!=0) //if not in main menu
		{
			...
			if(m_nDyingFrameCount>1&&Timer.Elapsed(m_nDyingLastFrameTime,t))
				if(++m_nDyingCurrentFrame>=m_nDyingFrameCount) 
				{
					...
					if(m_enUpType!=NOUP)
						UpManager.Create(m_enUpType,m_nX,m_nY);
				}
		}
	}
}
...
void CObject::Move(DWORD time)
{
	...
	int xdelta, ydelta;
	int tfactor;
	int fire_target_x, fire_target_y;
	...
	switch(m_enObjectType)
	{
		case ORANGE_BLIP_OBJECT: case GREEN_BLIP_OBJECT:
			if(m_bDying||m_bDead)
				return;
			break;
		case BLUE_SLITH_OBJECT: case GOLD_SLITH_OBJECT:
			if(m_bDying||m_bDead)
				return;
			break;
		case SPARK_OBJECT:
	...
	//evaluate new position (check bounds)
	switch(m_enObjectType)
	{
	...
	{
		//if object target is ship
		if(m_pEventList[m_nEventIndex]->iTargetX<0)
		{
			if(ReachedDestination(Ship.GetX(), Ship.GetY(),	m_nX, m_nY, m_nLastX, m_nLastY))
			{
				Kill();
			}
			AlterVector(m_nVectorX, m_nVectorY, m_nX, m_nY, Ship.GetX(), Ship.GetY(), m_pEventList[m_nEventIndex]->iVectorSpeed);
		}
		//else its a fixed target
		else
		{
	...
	//specific to FireWeapon event type
	else if(m_pEventList[m_nEventIndex]->bFireWeapon)
	{
		//create a new fire object...
		
		//determine target x and y
		if(m_pEventList[m_nEventIndex]->iTargetX<0)
			fire_target_x=Ship.GetX();
		else
			fire_target_x=m_pEventList[m_nEventIndex]->iTargetX;
		if(m_pEventList[m_nEventIndex]->iTargetY<0)
			fire_target_y=Ship.GetY();
		else
			fire_target_y=m_pEventList[m_nEventIndex]->iTargetY;

		//create it
		FireMan.Create(ENEMY_FIRE_ONE,m_nX,m_nY,fire_target_x,fire_target_y,m_pEventList[m_nEventIndex]->iVectorSpeed);

		//move on to next event immediatly
		m_nEventIndex=m_pEventList[m_nEventIndex]->iNextEventIndex;
	}
}
