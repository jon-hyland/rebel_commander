...
CObject::CObject(ObjectType object, SpriteType object_sprite, SpriteType dying_sprite, int x, int y, ObjEventItem **event_list)
{
	...
	m_nCurrentFrame=0;
	m_nLastFrameTime=Timer.time();
	m_bAnimateBack=FALSE;
	m_bDying=FALSE; m_bDead=FALSE;
	m_bPivotAnimation=TRUE;
	m_nDyingCurrentFrame=0; m_nDyingFrameCount=0; m_nDyingLastFrameTime=0;
...
void CObject::Draw(LPDIRECTDRAWSURFACE surface)
{
	...
	//int t=m_nFrameInterval/(1+abs(m_nVectorX));
...
/*void CObject::Move()
{
	...
	xdelta=(m_nVectorX*tfactor)/XSCALE;
	m_nX+=xdelta;
	...
	ydelta=(m_nVectorY*tfactor)/YSCALE;
	m_nY+=ydelta;
