...
if (FAILED(dest->Blt(&dest_rect, bob->images[bob->curr_frame],
          ...
          NULL)))
    return(0);
/*if (FAILED(dest->BltFast(bob->x,bob->y,bob->images[bob->curr_frame],
          &source_rect,(DDBLTFAST_WAIT  | DDBLTFAST_SRCCOLORKEY ))))
    return(0);*/
...
int Draw_BOB(BOB_PTR bob, LPDIRECTDRAWSURFACE4 dest, int transparent, int x, int y)
{
	...
	int lasty=bob->y;
	Set_Pos_BOB(bob,x,y);
	Draw_BOB(bob,dest,transparent);
	Set_Pos_BOB(bob,lastx,lasty);
	return 1;
}
...
DrawText(xdc,text,strlen(text),&rect,0);
...
fflush(fp_error);	//<JOEY>flush err file</JOEY>
// return success
return(1);
} // end Write_Error
