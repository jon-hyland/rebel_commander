...
CBmpSpriteFileReader g_cGrayAlphaSprites;
...
BOOL LoadGraphics()
{
	...
	//load gray alpha sprites
	if(!g_cGrayAlphaSprites.load("gray_alpha.bmp")) return FALSE;
	g_pSprite[GRAY_ALPHA_SPRITE]=new CClippedSprite(40,16,16);
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,0,1,1)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,1,18,1)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,2,35,1)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,3,52,1)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,4,69,1)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,5,86,1)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,6,103,1)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,7,1,18)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,8,18,18)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,9,35,18)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,10,52,18)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,11,69,18)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,12,86,18)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,13,103,18)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,14,1,35)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,15,18,35)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,16,35,35)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,17,52,35)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,18,69,35)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,19,86,35)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,20,103,35)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,21,1,52)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,22,18,52)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,23,35,52)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,24,52,52)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,25,69,52)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,26,86,52)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,27,103,52)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,28,1,69)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,29,18,69)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,30,35,69)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,31,52,69)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,32,69,69)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,33,86,69)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,34,103,69)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,35,1,86)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,36,18,86)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,37,35,86)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,38,52,86)) return FALSE;
	if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,39,69,86)) return FALSE;
...
BOOL RestoreSurfaces()
{
	...
	//restore red alpha
	if(g_pSprite[GRAY_ALPHA_SPRITE]->Restore())
	{
		if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,0,1,1)) return FALSE;
		if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,1,18,1)) return FALSE;
		if(!g_pSprite[GRAY_ALPHA_SPRITE]->load(&g_cGrayAlphaSprites,2,35,1)) return FALSE;
