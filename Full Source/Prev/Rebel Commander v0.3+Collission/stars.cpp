...
void CStars::draw(DDSURFACEDESC *ddsd)
{
	for(int i=0; i<m_iStarCount; i++)
	{
		...
		{
			case SMALL_STAR:
				PutPx(m_iStarArray[i].x+SCREEN_OFFSET, m_iStarArray[i].y, m_iStarArray[i].pal_index, ddsd);
				break;
			case LARGE_STAR:
				PutPx(m_iStarArray[i].x+SCREEN_OFFSET, m_iStarArray[i].y, m_iStarArray[i].pal_index, ddsd);
				PutPx(m_iStarArray[i].x+SCREEN_OFFSET+1, m_iStarArray[i].y, m_iStarArray[i].pal_index, ddsd);
				PutPx(m_iStarArray[i].x+SCREEN_OFFSET, m_iStarArray[i].y+1, m_iStarArray[i].pal_index, ddsd);
				PutPx(m_iStarArray[i].x+SCREEN_OFFSET+1, m_iStarArray[i].y+1, m_iStarArray[i].pal_index, ddsd);
				break;
		}
	}
}
